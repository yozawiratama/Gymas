...
import { getRevenueReport } from '$lib/server/services/reportsService';
...
export const load: PageServerLoad = async (event) => {
	...
	try {
		...
		if (isAppError(err) && err.status === 400) {
			logger.warn('Revenue report validation failed', {
				route,
			...
			const fallback = await getRevenueReport({ branchId });
			return {
				...fallback,
		...
		logger.error('Revenue report load failed', {
			route,
			requestId: event.locals.requestId,
			error: err
		});
