...
<div class="min-h-screen bg-base-200">
	<div class="mx-auto flex max-w-3xl flex-col gap-6 p-6">
		<div class="prose max-w-none">
			<a class="link link-hover text-sm" href={`/members/${member().id}`}>Back to profile</a>
			<h1>Edit Member</h1>
			<p class="text-sm opacity-70">Update core member details safely.</p>
		</div>
		...
		<div class="card bg-base-100 shadow">
			<div class="card-body space-y-4">
				...
				<form method="POST" action="?/updateMember" class="space-y-4" onsubmit={lockSubmit}>
					...
					<div class="flex flex-wrap items-center gap-3">
						...
						<a class="btn btn-ghost" href={`/members/${member().id}`}>Cancel</a>
					</div>
				</form>
			</div>
		</div>
